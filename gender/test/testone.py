...
def loadmodel(pretrained_model='train.caffemodel',network='deploy.prototxt',size=256):
	...
	gender_net = caffe.Classifier(network, pretrained_model,mean=None,
					   channel_swap=(2,1,0),
					   raw_scale=255,
...
def batchclassify(filepath='test'):
	...
	os.mkdir(pickfile)
	model=loadmodel('model/gender.caffemodel','model/deploy.prototxt',90)
	imglists=os.listdir(filepath)
	count=0
	for imgpath in imglists:
